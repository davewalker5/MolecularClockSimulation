...
The Strict Molecular Clock Explorer provides an interactive environment for exploring the strict molecular clock simulation from the [_Molecular Clock Simulation_](https://github.com/davewalker5/MolecularClockSimulation) project and the complete workflow from sequence evolution to phylogenetic reconstruction.

Built on top of the project’s Strict Molecular Clock Engine, the explorer allows users to generate evolutionary histories, analyse simulated DNA sequences using multiple evolutionary distance models, visualise distance matrices and reconstruct phylogenetic trees using UPGMA or Neighbor Joining.
...
The following "docker run" parameters are recommended when running the strictclockexplorer image:
...
| --name     | strictclock | Name of the container once running                      |
| --rm       | -           | Remove the container automatically when it stops        |

For example:

```shell
docker run -d -p 80:8501 --platform linux/amd64 --name strictclock --rm davewalker5/strictclockexplorer:latest
```
